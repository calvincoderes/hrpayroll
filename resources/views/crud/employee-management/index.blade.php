...
<div class="module-header">
  <div class="row">
    ...
    <div class="col-xs-6 text-right"><a href="{{ url( $base, [ 'create' ] ) . $query_string }}" title="Add Employee" class="btn btn-primary"><i class="fa fa-plus"></i> Add Employee</a></div>
  </div>
</div>
...
<form action="#" method="post" enctype="multipart/form-data" id="form-filter">
  ...
  {{-- @foreach( $results as $result ) --}}
        <tr>
          ...
          <td class="text-left">Sample Data</td>
          <td class="text-right">
            <a href="#" class="btn btn-info" data-toggle="modal" title="{{ $text_show }}"><i class="fa fa-search"></i></a>
            <a href="#" title="{{ $text_edit }}" class="btn btn-primary"><i class="fa fa-pencil"></i></a>
            <a href="#" class="btn btn-danger remove" data-toggle="modal" title="{{ $text_remove }}"><i class="fa fa-times"></i></a>
          </td>
        </tr>
  {{-- @endforeach --}}
      </tbody>
...
<div class="row">
    <div class="col-sm-12 text-center">Showing 1 of 1 entries</div>
...
@section('scripts')
<script src="{{ asset('js/crud/jquery.' . $base . '-list.js') }}" type="text/javascript"></script>
@stop
